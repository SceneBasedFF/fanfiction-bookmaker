# Story Repo YAMLs

These are the YAML files you need in your writing repo.

Files in this folder:

- `fanfic.yml` - optional book-level settings
- `chapters.yml` - chapter order
- `chapters/example-chapter.yml` - chapter structure template
- `.github/workflows/build-books.yml` - GitHub Actions workflow

Copy the files into your story repo and rename the chapter template files to match your actual chapter slugs.
